...
from tetris.utils import rectangle
...
class Tetris:
    ...
    def get_less_pos(self):
        if self.block is not None:
            temp_pos = self.pos.copy()
            self.del_forme(self.pos, self.block)
            while True:
                if temp_pos[0] + len(self.block) >= len(self.grille):
                    break
                if not self.collision((temp_pos[0] + 1, temp_pos[1]), self.block):
                    temp_pos[0] += 1
                else:
                    break
            self.add_forme(self.pos, self.block)
    ...
    def tp_less(self):
        if self.block:
            temp_pos = self.pos
            self.pos = self.get_less_pos()
            self.del_forme(temp_pos, self.block)
            self.add_forme(self.pos, self.block)
            self.check_full_line()
            self.block = None
            self.add_block()
    ...
    def print_less_pos(self):
        if self.block:
            temp_pos = self.get_less_pos()
